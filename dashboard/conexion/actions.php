...
if (isset($_POST['action']) && $_POST['action'] == 'delfav') {
    $id_curso = $_POST['idCurso'];
    $id_user = $_POST['idUser'];

    try {
        $delFav = $actions->delData('fav', 'id_curso', 'id_usuario', $id_curso, $id_user, 'ii');
        $response = array(
            "success" => true,
            "message" => ($delFav) ? "El curso se a eliminado de favoritos" : "No se pudo eliminar el curso de favoritos",
        );
        echo json_encode($response);
    } catch (\Throwable $th) {
        $response = array('success' => false, 'message' => 'Error al eliminar el curso: ' . $th->getMessage());
        echo json_encode($response);
    }
}
        ...
        if (!$sql_s_o) {
            try {
                ...
                if (in_array($fileExtension, $extension_permitida)) {
                    ...
                    if (move_uploaded_file($fileTmpName, $destino)) {
                        // insertar acá{
                        $camposdb = 'id_curso, titulo, video_url, orden, active, img_leccion';
                        $val = "?, ?, ?, ?, ?, ?";
                        $bind = "issiis";
                        // $data = array($nombre, $nuevo_nombre, $instructor, $fechadb, $estado, $tipo);
                        $data = array($id_curso, $nombre, $video_url, $orden, $estado, $nuevo_nombre);
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
        ...
        try {
            ...
            if (in_array($fileExtension, $extension_permitida)) {
                ...
                $destino = '../assets/img_leccion/' . $nuevo_nombre;
                if (move_uploaded_file($fileTmpName, $destino)) {
                    $update_curso = $actions->updateLeccion($name, $nuevo_nombre, $video_url, $idleccionhtml);
                    if ($update_curso) {
